import Link from "next/link";
import { notFound } from "next/navigation";
import { Pencil } from "lucide-react";
import { requireUser } from "@/lib/auth/guard";
import { getCustomerForUser } from "@/server/services/customer-service";
import { ServiceError } from "@/server/services/errors";
import { canManageCustomers } from "@/lib/permissions";
import { CUSTOMER_TYPE_META } from "@/components/app/customer-type";
import { PROJECT_STATUS_META } from "@/components/app/project-status";
import { CustomerArchiveForm } from "@/components/app/customer-archive-form";
import { cn } from "@/lib/utils";

export default async function CustomerDetailPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const { id } = await params;
  const user = await requireUser();

  let customer;
  try {
    customer = await getCustomerForUser(user, id);
  } catch (err) {
    if (err instanceof ServiceError && err.code === "NOT_FOUND") notFound();
    throw err;
  }

  const type = CUSTOMER_TYPE_META[customer.type] ?? { label: customer.type, tone: "bg-muted text-muted-foreground" };
  const canManage = canManageCustomers(user.role);

  return (
    <div className="mx-auto max-w-4xl animate-fade-in space-y-6">
      <div className="flex flex-wrap items-start gap-3">
        <div>
          <h1 className="font-display text-2xl font-bold tracking-tight">{customer.name}</h1>
          <div className="mt-1 flex items-center gap-2">
            <span className={cn("inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[12px] font-semibold", type.tone)}>
              {type.label}
            </span>
            <span
              className={cn(
                "inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[12px] font-semibold",
                customer.isActive ? "bg-success/12 text-success" : "bg-destructive/10 text-destructive",
              )}
            >
              {customer.isActive ? "Aktif" : "Arşivlendi"}
            </span>
          </div>
        </div>
        {canManage && (
          <Link
            href={`/customers/${customer.id}/edit`}
            className="ml-auto inline-flex h-9 items-center gap-1.5 rounded-lg border border-border bg-card px-3.5 text-[13px] font-semibold text-foreground shadow-sm transition-colors hover:bg-muted"
          >
            <Pencil className="h-3.5 w-3.5" />
            Düzenle
          </Link>
        )}
      </div>

      <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-5 shadow-soft">
            <h2 className="font-display text-[15px] font-semibold tracking-tight">Genel bakış</h2>
            <dl className="mt-4 grid grid-cols-2 gap-4 text-sm">
              <Field label="TCKN / VKN" value={customer.identityOrTaxNumber ?? "—"} />
              <Field label="Vergi dairesi" value={customer.taxOffice ?? "—"} />
              <Field label="İlgili kişi" value={customer.contactName ?? "—"} />
              <Field label="Telefon" value={customer.phone ?? "—"} />
              <Field label="E-posta" value={customer.email ?? "—"} />
              <Field label="Konum" value={[customer.city, customer.district].filter(Boolean).join(" / ") || "—"} />
            </dl>
            {customer.address && <p className="mt-4 text-sm text-muted-foreground">{customer.address}</p>}
          </div>

          <div className="rounded-2xl border border-dashed border-border bg-card p-5">
            <h2 className="font-display text-[15px] font-semibold tracking-tight">Alacak özeti</h2>
            <p className="mt-1 text-sm text-muted-foreground">
              Toplam sözleşme bedeli, tahsil edilen ve kalan alacak; gelir ve tahsilat modülleri eklendiğinde burada
              görünecektir.
            </p>
          </div>

          {canManage && (
            <div className="rounded-2xl border border-border bg-card p-5 shadow-soft">
              <h2 className="font-display text-[15px] font-semibold tracking-tight">Kayıt durumu</h2>
              <p className="mt-1 text-sm text-muted-foreground">
                {customer.isActive
                  ? "Bu müşteri arşivlenirse yeni projelerde seçilemez."
                  : "Bu müşteri arşivde; yeniden etkinleştirebilirsiniz."}
              </p>
              <div className="mt-3">
                <CustomerArchiveForm customerId={customer.id} isActive={customer.isActive} />
              </div>
            </div>
          )}
        </div>

        <div className="rounded-2xl border border-border bg-card p-5 shadow-soft">
          <h2 className="font-display text-[15px] font-semibold tracking-tight">İlişkili projeler</h2>
          {customer.projects.length === 0 ? (
            <p className="mt-2 text-sm text-muted-foreground">Bu müşteriye bağlı proje yok.</p>
          ) : (
            <ul className="mt-3 space-y-1.5">
              {customer.projects.map((p) => {
                const st = PROJECT_STATUS_META[p.status];
                return (
                  <li key={p.id}>
                    <Link
                      href={`/projects/${p.id}`}
                      className="flex items-center justify-between gap-2 rounded-lg border border-border px-3 py-2 transition-colors hover:bg-muted/50"
                    >
                      <div className="min-w-0">
                        <p className="truncate text-[13px] font-medium">{p.name}</p>
                        <p className="text-[11px] text-muted-foreground">{p.code}</p>
                      </div>
                      <span className={cn("shrink-0 rounded-full px-2 py-0.5 text-[11px] font-semibold", st.tone)}>{st.label}</span>
                    </Link>
                  </li>
                );
              })}
            </ul>
          )}
        </div>
      </div>
    </div>
  );
}

function Field({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <dt className="text-[11px] text-muted-foreground">{label}</dt>
      <dd className="mt-0.5 font-medium">{value}</dd>
    </div>
  );
}
