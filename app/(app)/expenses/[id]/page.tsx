...
import { canCancelFinancialRecord, canManageExpenses, canRecordSettlement } from "@/lib/permissions";
...
export default async function ExpenseDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const canEdit = canManageExpenses(user.role) && record.status !== "CANCELLED" && record.settledAmount.isZero();
        ...
        <div>
          ...
          <div className="mt-1 flex items-center gap-2">
            <span className={cn("inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[12px] font-semibold", status.tone)}>
              {transactionStatusLabel(record.status, "EXPENSE")}
            </span>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-5 shadow-soft">
            ...
            <dl className="mt-4 grid grid-cols-2 gap-4 text-sm">
              ...
              <Field label="Tedarikçi" value={record.supplier?.name ?? "—"} />
          ...
          <div className="rounded-2xl border border-border bg-card p-5 shadow-soft">
            <h2 className="font-display text-[15px] font-semibold tracking-tight">Ödemeler</h2>
            {canSettle && (
              <div className="mt-3">
                <SettlementForm
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-5 shadow-soft">
            ...
            <dl className="mt-4 space-y-3 text-sm">
              ...
              <div className="flex justify-between">
                <dt className="text-muted-foreground">Kalan borç</dt>
              ...
              {canCancel ? (
                <div className="mt-3">
                  <CancelReasonForm
                    ...
                    warningText="Bu gider kaydı iptal edilecektir. İptal edilen kayıt aktif toplamlara dahil edilmez."
                  />
                </div>
              ) : (
                <p className="mt-2 text-[12px] text-muted-foreground">
                  Bu kayda ödeme girildiği için doğrudan iptal edilemez; önce ödemeleri iptal edin.
                </p>
              )}
            </div>
          )}
