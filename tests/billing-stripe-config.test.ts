...
describe("YF-808 — Stripe yapılandırması: fail-closed", () => {
  ...
  it("fiyat çözümlemesi de gizli anahtar yokken fail-closed reddedilir", () => {
    setEnv({ STRIPE_PRICE_STARTER: "price_FAKEstarter001" });
    expect(() => resolveStripePriceForPlan("STARTER")).toThrow(BillingConfigError);
  });
  ...
  it("STRIPE_PRICE_* alanına gizli anahtar yazılamaz (biçim reddi)", () => {
    setEnv({ STRIPE_SECRET_KEY: TEST_SECRET_KEY, STRIPE_PRICE_STARTER: TEST_SECRET_KEY });
...
describe("YF-808 — test/live ortam ayrımı sessizce karışamaz", () => {
  ...
  it("çözümlenen fiyat her zaman geçerli ortamı taşır (eşleme ortamdan bağımsız kullanılamaz)", () => {
    setEnv({ STRIPE_SECRET_KEY: LIVE_SECRET_KEY, STRIPE_PRICE_BUSINESS: "price_FAKEbusiness001" });
    const price = resolveStripePriceForPlan("BUSINESS");
...
describe("YF-808 — plan → Stripe Price eşlemesi", () => {
  ...
  it("env fiyat haritası her kanonik planı kapsar (sessiz kayma olamaz)", () => {
    setEnv({ STRIPE_SECRET_KEY: TEST_SECRET_KEY });
    const priceKeys = Object.keys(getEnv().stripe.prices).sort();
    expect(priceKeys).toEqual([...CANONICAL_BILLING_PLAN_CODES].sort());
  });

  it("yapılandırılmış bir plan için Price ID doğru çözümlenir", () => {
    setEnv({ STRIPE_SECRET_KEY: TEST_SECRET_KEY, STRIPE_PRICE_PROFESSIONAL: "price_FAKEprofessional001" });
    const price = resolveStripePriceForPlan("PROFESSIONAL");
  ...
  it("bilinmeyen/eşlenmemiş plan kodu fail-closed reddedilir", () => {
    setEnv({ STRIPE_SECRET_KEY: TEST_SECRET_KEY, STRIPE_PRICE_STARTER: "price_FAKEstarter001" });
    expect(() => resolveStripePriceForPlan("PLATINUM")).toThrow(BillingConfigError);
    expect(() => resolveStripePriceForPlan("PLATINUM")).toThrow(/Bilinmeyen plan kodu/);
    // Kanonik ama yapılandırılmamış plan da sessizce geçilmez.
    expect(() => resolveStripePriceForPlan("BUSINESS")).toThrow(/STRIPE_PRICE_BUSINESS yapılandırılmamış/);
  });

  it("Enterprise contact-sales ise UYDURMA bir Price ID döndürülmez", () => {
    setEnv({ STRIPE_SECRET_KEY: TEST_SECRET_KEY, STRIPE_PRICE_ENTERPRISE: CONTACT_SALES_SENTINEL });
    const price = resolveStripePriceForPlan("ENTERPRISE");
    expect(price.kind).toBe("CONTACT_SALES");
    expect(price.priceId).toBeNull();
  });
