      ...
      {rows.length === 0 ? (
        <div className="grid place-items-center rounded-2xl border border-dashed border-border bg-card py-16 text-center">
          ...
          <p className="mt-3 text-sm text-muted-foreground">
            {suppliers.length === 0 ? "Henüz tedarikçi eklenmemiş." : "Bu filtreyle eşleşen tedarikçi yok."}
          </p>
        </div>
      ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border text-left">
                  <th className="label-mono py-2.5 pl-4 font-medium text-muted-foreground">Tedarikçi</th>
              ...
              <tbody>
                {rows.map((s) => {
                  const type = SUPPLIER_TYPE_META[s.type] ?? { label: s.type, tone: "bg-muted text-muted-foreground" };
                  return (
                    <tr
                      key={s.id}
                    ...
                    >
                      <td className="py-3 pl-4">
                        <Link href={`/suppliers/${s.id}`} className="flex items-center gap-3">
                          ...
                          <p className="min-w-0 truncate font-semibold leading-tight">{s.name}</p>
                        </Link>
                      </td>
                      ...
                      <td className="py-3 pr-4 text-[13px] text-muted-foreground">{s.contactName ?? "—"}</td>
                      <td className="py-3 pr-4 text-[13px] text-muted-foreground">{s.phone ?? "—"}</td>
                      <td className="py-3 pr-4">
                        <span
                          className={cn(
                            "inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-semibold",
                            s.isActive ? "bg-success/12 text-success" : "bg-destructive/10 text-destructive",
                          )}
                        >
                          {s.isActive ? "Aktif" : "Arşivlendi"}
                        </span>
                      </td>
                    </tr>
                  );
