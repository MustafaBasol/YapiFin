...
- [ ] **Monitoring yapılandırması (YF-512)**: `SENTRY_DSN` bilinçli olarak ayarlandıysa (🔧 operatör kararı — zorunlu değildir), süreç başlangıç loglarında `monitoring.not_configured` uyarısı **görünmemeli**. `SENTRY_DSN` bilinçli olarak boş bırakıldıysa bu uyarının bir kez göründüğü doğrulanır (sır içermez). ⚠️ `scripts/trigger-test-monitoring-event.ts` production'da **çalıştırılmaz** (script kendisi reddeder) — Sentry entegrasyonunun uçtan uca doğrulaması yalnızca development/staging'de yapılır (bkz. [MONITORING_RUNBOOK.md — Kontrollü test olayı](./MONITORING_RUNBOOK.md#kontrollü-test-olayı)).
