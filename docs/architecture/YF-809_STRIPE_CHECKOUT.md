...
## 8. Mükerrer deneme koruması ve idempotency
...
- **Stripe tarafı idempotency:** deterministik anahtar
  ...
  Stripe'ta güvenle YENİ bir oturum üretir.
- **DB tarafı:** `create` P2002 ile çakışırsa (organizasyonun satırı zaten
  var), çakışan satır yalnızca (a) AYNI plan/aralık/ortamı taşıyorsa (kendi
  tekrarımız — hızlı çift tıklama), (b) süresi dolmuşsa VEYA (c) FARKLI bir
  Stripe ortamına aitse (bir dağıtımın tek ortamı olabileceği için ortam
  uyuşmazlığı her zaman bayat sayılır) üzerine yazılır; aksi halde
  `ServiceError(CONFLICT)` ile reddedilir.
- **Bilinen sınır:** ön-kontrol (Stripe çağrısından ÖNCEKİ okuma) ile DB
  yazımı arasında, aynı organizasyon için gerçekten eşzamanlı FARKLI plan
  istekleri teorik olarak her ikisi de birer Stripe Checkout Session'ı
  oluşturabilir (yalnızca biri DB'de izlenir, diğeri Stripe tarafında 24 saat
  içinde kendiliğinden sona erer — hiçbir ücret/abonelik OLUŞTURMAZ). AYNI
  plan+aralık için bu risk yoktur (Stripe idempotency anahtarı tam olarak
  çözer). Dağıtık kilit GEREKTİRMEYEN, bilinçli olarak minimal bir tasarım
  kararıdır (bkz. `server/services/billing/checkout-service.ts` dosya başı
  not).

Deterministik eşzamanlılık testleri (sleep KULLANMAZ, `Promise.all`):
`tests/billing-checkout.test.ts` "hızlı çift tıklama" ve "aynı organizasyon
FARKLI bir plan için açık deneme varken CONFLICT" senaryoları.
...
`prisma/migrations/20260810150000_yf809_stripe_checkout` — additive:
